...
|---|---|---|---|
| ST | TSSOP-14 | 14 | No |
| MQ | VQFN-20 | 20 + EP (5 mm x 5 mm) | Yes (EP = pin 21, connected to V-) |
...
- Significantly improves thermal performance: thetaJA = 36.1  degC/W vs 100  degC/W for TSSOP-14.
...
|---|---|---|---|---|---|
| Schmitt Trigger High-Input Threshold | VIH | 0.7 x VL | VL + 0.3V | V | 1.8V <= VL <= 5.5V |
| Schmitt Trigger Low-Input Threshold | VIL | DGND - 0.5V | 0.3 x VL | V | - |
| Hysteresis | VHYS | 0.1 x VL | - | V | - |
| Output Low Voltage (SDA only) | VOL | DGND | 0.2 x VL | V | VL = 5.5V, IOL = 5 mA |
| Output Low Voltage (SDA only) | VOL | DGND | 0.2 x VL | V | VL = 1.8V, IOL = 800 uA |
| Input Leakage Current | IIL | -1 | +1 | uA | VIN = VL or VIN = DGND |
...
| Max Terminal Current (-502, 5 kOhm) | IT | 25 | mA | IBW (W != ZS) and IAW (W != FS) |
| Max Terminal Current (-103, 10 kOhm) | IT | 12.5 | mA | - |
| Max Terminal Current (-503, 50 kOhm) | IT | 6.5 | mA | - |
| Max Terminal Current (-104, 100 kOhm) | IT | 6.5 | mA | - |
...
- No exposed pad.
- thetaJA = 100  degC/W, max power dissipation at TA = +50 degC, TJ = +150 degC: 1000 mW.

[Source: DS20005304B, p.3, p.80-95]

### VQFN-20 (MQ)
- 20 leads + exposed pad (EP, pin 21), 5 mm x 5 mm body.
...
- thetaJA = 36.1  degC/W, max power dissipation at TA = +50 degC, TJ = +150 degC: 2800 mW.
