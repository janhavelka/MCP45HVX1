...
**Source document:** `docs/MCP45HVX1-Data-Sheet-DS20005304.pdf`, DS20005304B Rev B, June 2023 (Microchip Technology Inc.)
**Raw extract:** `docs/pdf-extracted-md/MCP45HVX1-Data-Sheet-DS20005304.md`, pages 26-80
...
The wiper position is undefined until POR completes. After POR, the wiper defaults to mid-scale. The I2C interface becomes active after POR. No explicit startup delay time (tPOR) is quantified in the extracted pages. [Source: DS20005304B, p. 28]
...
Purpose: Reset the MCP45HVX1 I2C state machine to a known state. Use when the device may have latched into an unknown I2C state (e.g., after host reset during a transaction, or after bus noise).

**Does NOT reset register values.** Only the I2C state machine is reset. [Source: DS20005304B, p. 66]
...
### Effect on other I2C devices

This sequence is described as an invalid command to all standard I2C devices; they should disregard it. [Source: DS20005304B, p. 66]
...
4. The I2C interface is ready to accept commands immediately after POR.
...
> Warning **Conflict:** TABLE 1-2 spec 95 specifies **250 ns minimum** hold time (TWLHD); Note 9 describes the indeterminate window as 10 ns before to **200 ns** after. A safe implementation must use 250 ns. [Conflict: DS20005304B, TABLE 1-2 Spec 95, p. 17 vs. Note 9, p. 17]
...
|---|---|---|
| VL | 0.1 uF | Within 4 mm of the VL pin |
| V+ | 0.1 uF | As close as possible |
...
| Package | Dimensions (mm) | Area (mm2) | Relative Area |
|---|---|---|---|
| 14-TSSOP (ST) | 5.10 x 6.40 | 32.64 | 1.31x |
| 20-VQFN (MQ) | 5.00 x 5.00 | 25.00 | 1x (smallest) |
...
| RAB Typical (Ohm) | RAB Min (Ohm) | RAB Max (Ohm) | |VA|+|VB| (V) | Power (mW) |
|---|---|---|---|---|
...
Power = V x I. [Source: DS20005304B, p. 74]

> The 900 mW figure for the 5 kOhm variant at full voltage swing must be checked against the device's thermal limits (thetaJA) to ensure junction temperature stays within -40 degC to +125 degC. See `03_electrical_and_timing.md` for thermal resistance values.
...
```
8-bit: VOUT(N) = (R1/R2) x (VD x (1 + N/255))    N = 0 to 255
7-bit: VOUT(N) = (R1/R2) x (VD x (1 + N/127))    N = 0 to 127
```
...
```
8-bit: Gain(N) = 1 + (49.4 kOhm) / ((N/255) x RAB)    N = 0 to 255
7-bit: Gain(N) = 1 + (49.4 kOhm) / ((N/127) x RAB)    N = 0 to 127
```
...
```
8-bit: VOUT(N) = 1.23V x (1 + (R2 / (N x RAB/255)))    N = 0 to 255
7-bit: VOUT(N) = 1.23V x (1 + (R2 / (N x RAB/127)))    N = 0 to 127
```
