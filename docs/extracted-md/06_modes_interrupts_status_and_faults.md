...
**Raw extract:** `docs/pdf-extracted-md/MCP45HVX1-Data-Sheet-DS20005304.md`, pages 26-55
...
The I2C serial interface remains active regardless of SHDN or TCON state. [Source: DS20005304B, p. 47]
...
- The I2C serial interface remains **fully active**.
...
|---|---|---|---|---|
| VIH (SHDN) | 0.7 x VL | - | VL + 0.3 | V |
| VIL (SHDN) | -0.3 | - | 0.3 x VL | V |
...
**R0HW = 0** produces the same physical result as the SHDN hardware pin asserted Low - but via software. This allows shutdown control entirely through I2C without using the SHDN pin. [Source: DS20005304B, p. 47]
...
> Warning **WLAT Hold Time Conflict within DS20005304B:**
>
> - **TABLE 1-2, Specification 95 (p. 17):** TWLHD (WLAT hold time after SCL rising edge) = **250 ns min** (no max given; VL = 2.7-5.5 V, TA = -40 degC to +125 degC)
...
WLAT allows simultaneous wiper updates across multiple MCP45HVX1 devices: update all devices via I2C while WLAT is High, then simultaneously release WLAT on all devices to apply the changes. This achieves glitch-free synchronous updates (analogous to LDAC on multi-channel DACs). [Source: DS20005304B, p. 27, p. 50]
...
A GCEN (General Call Enable) bit controls whether the device responds to the I2C General Call address (0x00).
...
After a NACK due to invalid command, all subsequent commands are **ignored** until the host issues a new Start condition to reset the I2C state machine. [Source: DS20005304B, p. 58]
...
If the MCP45HVX1 receives an incomplete or undefined I2C transmission, it:
...
|---|---|---|---|
| Normal operation (VL = 5V, TSSOP, 25 degC) | IDD (VL) | 1 uA | 5 uA |
| Normal operation (V+/V-) | IDD (V+) | - | +/- [see file 03] |
| SHDN asserted | IDD (VL) | Same (I2C active) | - |

[Source: DS20005304B, p. 12-14]

> The SHDN pin does not reduce digital (VL) supply current; the I2C interface remains powered and active. [Source: DS20005304B, p. 47]
